...
function checkStructure() {
    ...
    if (array_key_exists('Login', $data) &&
        ...
        if (!empty($data['Rank']) && !empty($data['Login']) && !empty($data['Password']))  {
            return true;
        } else return false;
    } else return false;
}
...
if (enter_to_view_by_rank($sql_cursor, 1) && checkStructure()) {
    ...
    $sql_cursor->query(
        "INSERT INTO Users ( Login, Password, Email, Rank_type) values ('{$data['Login']}','{$data['Password']}', '{$data['Email']}', {$data['Rank']} )"
    );
